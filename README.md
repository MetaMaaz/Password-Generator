...
```bash
git clone https://github.com/MetaMaaz/Password-Generator.git
...
- **Why `secrets` over `random`** — `random` is seeded from predictable state and is not suitable for security-sensitive output. `secrets` pulls from `/dev/urandom` (or the OS equivalent) and is cryptographically sound.
- **Why k-anonymity** — the HIBP range API lets us check if a password has been breached without ever sending the full hash. We send only the first 5 hex chars of the SHA-1 hash; HIBP returns all suffixes that match, and we check locally.
